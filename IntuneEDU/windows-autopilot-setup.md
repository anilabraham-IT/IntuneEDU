...
1. Make sure your devices meet [the software, networking, licensing, and configuration requirements](https://docs.microsoft.com/windows/deployment/windows-autopilot/windows-autopilot-requirements) for Windows Autopilot.      
...
5. Connect to a Wi-Fi network with internet access. Alternatively, connect to a wired network.   
...
4. By default, a standard user account type is selected. If you want all users in your tenant to be admins on their devices, select **Administrator**.  
...
4. For the remaining options on the screen:
    a. Choose **Windows**. 
    b. Choose **Device group tag starts with**.
5. Type in the name of the group tag. For example, Contoso Teachers.
6. Select **Create group**.  

The group will populate with all Autopilot-registered, student devices that have the group tag. Next, go to Settings > Windows Device Settings > Enrollment > Windows Autopilot to assign a deployment profile to the group.

## Configure settings and apps  
If each device in your school is used by a single user, and not shared, we recommend that you:  
...
If devices in your school are shared by more than one user, configure all common settings and apps under the all devices group. When you need to be more specific about the apps and settings that are on a device, go to the appropriate device group and configure them there. 

## Distribute devices  
After you’re done with the configuration, and devices are delivered to each school, distribute them to students and teachers for setup. Students can set up the devices at home or at school, as long as there is a reliable network. For a brief overview of what the out-of-box setup experience is like, see [User-driven mode](#user-driven-mode).    
